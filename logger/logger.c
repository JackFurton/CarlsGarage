...
void log_log(int level, const char *file, int line, const char *fmt, ...) {
    ...
    if (!log_global_cfg.quiet && level >= log_global_cfg.level) {
        ...
        // in simple terms, ev.ap (we can call it our log_event_t structs argument list)
        // is uninitialized and needs to be populatd with a variable amount of args
        ...
        // and populate (ev.ap), our arg list
        va_start(ev.ap, fmt);
